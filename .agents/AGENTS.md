...
8. **Defensive CSV Parsing**: Raw bank statements often have poorly formatted headers. When reading raw data with Polars (`pl.read_csv`), always strip whitespace from column names immediately (e.g., `df.rename({col: col.strip() for col in df.columns})`) to prevent `ColumnNotFoundError`.

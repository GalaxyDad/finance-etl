...
def test_call_gemini_categorization_personal_keywords(mock_data_dir, monkeypatch):
    ...
    assert "Personal Filtering Keywords (Soft filter these):" in prompt_used
    assert "custom_word_1" in prompt_used
    assert "custom_word_2" in prompt_used
    assert "custom_word_2" in prompt_used

def test_call_gemini_categorization_exclusion_keywords(mock_data_dir, monkeypatch):
    from unittest.mock import MagicMock
    from src.etl import call_gemini_categorization
    import os
    ...
    unique_merchants = ['TEST MERCHANT']
    exclusion_keywords = ['exclusion_1', 'exclusion_2']
    ...
    # Call function
    call_gemini_categorization(unique_merchants, reference_dir, personal_items_profile=[], exclusion_keywords=exclusion_keywords)
    
    # Extract the prompt used
    call_args = mock_client.models.generate_content.call_args
    prompt_used = call_args.kwargs['contents']
    
    assert "Personal Filtering Exclusions (NEVER soft filter these):" in prompt_used
    assert "exclusion_1" in prompt_used
    assert "exclusion_2" in prompt_used
